...
The computer has internal memory arranged into rows of 128 quasi-32-bit cells, `mem_cells` cells and `mem_rows` rows overall. These values are configurable at creation time. For illustrative purposes, also keep the number `mem_p2rows` in mind, which is the smallest whole power of 2 larger than or equal to `mem_rows`, and the corresponding number of cells, `mem_p2cells`. The 16-bit address space is divided into 512 128-cell blocks, which are mapped to the internal memory as follows:
 ...
 - `mem_p2rows - mem_rows` blocks are mapped to the highest-address row of the internal memory in read-only mode
...
Only one of the external/internal read/write bits is ever set in any given frame.
...
The computer has a 16-bit program counter, which always points at the next instruction. When the computer is running, whenever an instruction is executed, it is fetched from memory from whatever address the program counter points at. The program counter is then increased by 1 (possibly overflowing to 0) and the execution of the next instruction follows. When the computer is not running, the program counter stays unchanged. Assuming no user interaction, all other computer state stay unchanged.
...
Conditions, expressed in terms of the four flags `Zf`, `Sf`, `Cf`, and `Of`, are as follows:
...
Feel free to ignore this section if you do not plan on making your own custom R316-based saves.
...
Boolean, optional, defaults to `false`. If `true`, both the screen and the keyboard must be requested, and they share a body.
...
Integers, exactly one of the two required. This determines the horizontal placement of the given border (left or right) of the termination component.
...
Integers, exactly one of the two required. This determines the horizontal placement of the given border (left or right) of a port component.
...
Integers, exactly one of the two required. This determines the horizontal placement of the given border (left or right) of the breakout box.
