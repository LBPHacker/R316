...
return testbed.module({
	...
	inputs = {
		...
		{ name = "ram_data_prev", index = 45, keepalive = 0x00000000, payload = 0xFFFFFFFF, never_zero = true, initial = 0xDEADBEEF },
		{ name = "ram_addr_prev", index = 47, keepalive = 0x10000000, payload = 0x000FFFFF,                    initial = 0x10000000 },
		{ name = "retry"        , index = 43, keepalive = 0x00000002, payload = 0x00000001,                    initial = 0x00000002 },
	},
	outputs = {
		...
		{ name = "ram_data_prev"   , index = 45, keepalive = 0x00000000, payload = 0xFFFFFFFF, never_zero = true },
		{ name = "ram_addr_prev"   , index = 47, keepalive = 0x10000000, payload = 0x000FFFFF,                   },
	},
	func = function(inputs)
		local ram_addr_r, ram_data_r = spaghetti.select(
			inputs.retry:band(1):zeroable(),
			inputs.ram_addr_prev, inputs.ram_addr,
			inputs.ram_data_prev, inputs.ram_data
		)
		ram_data_r:never_zero()
		local ram_data = ram_data_r:bor(0x10000000):band(0x3FFFFFFF)
		local addressed = ram_addr_r:bxor(inputs.base_addr)
		local registers = addressed:bsub(0x08):bsub(0x10):bxor(0x40)
		local prints    = addressed:bsub(0x10):bsub(0x0F)
		local next_char0_left  = spaghetti.select(registers                      :bsub(0x10000000):zeroable(), inputs.char0_left , ram_data_r):never_zero()
		local next_char0_right = spaghetti.select(registers:bxor(0x01)           :bsub(0x10000000):zeroable(), inputs.char0_right, ram_data_r):never_zero()
		...
		local next_scrollmask  = spaghetti.select(registers           :bxor(0x07):bsub(0x10000000):zeroable(), inputs.scrollmask , ram_data_r:bor(0x20000000):band(0x3FFFFFFF))
		...
		return {
			...
			pixel_color      = spaghetti.lshiftk(ram_addr_r:bor(0x10000), 4):bor(0x10000000),
			...
			char_config      = ram_addr_r,
			ram_data_prev    = inputs.ram_data,
			ram_addr_prev    = inputs.ram_addr,
		}
	end,
	fuzz_inputs = function()
		local retry = math.random(1, 2) == 1
		...
		local ram_addr_prev = bitx.bor(0x10000000, bitx.bor(math.random(0x00000000, 0x0000FFFF), bitx.band(0xF0000, bitx.lshift(0x10000, math.random(0, 4)))))
		if math.random(1, 2) == 1 then
			target_addr = bitx.band(retry and ram_addr_prev or ram_addr, 0xFFFF)
		end
		local base_addr = bitx.bor(0x00020000, bitx.band(target_addr, 0xFF80))
		return {
			ram_data      = testbed.any(),
			ram_addr      = ram_addr,
			ram_data_prev = testbed.any(),
			ram_addr_prev = ram_addr_prev,
			base_addr     = base_addr,
	...
	fuzz_outputs = function(inputs)
		local retry    = bitx.band(inputs.retry, 1) ~= 0
		local ram_data = retry and inputs.ram_data_prev or inputs.ram_data
		local ram_addr = retry and inputs.ram_addr_prev or inputs.ram_addr
		local address          = bitx.band(ram_addr, 0x7F)
		local addressed        = bitx.band(ram_addr, 0xFFF80) == inputs.base_addr
		...
		if addressed then
			...
			elseif bitx.band(address, 0x67) == 0x40 then
				next_char0_left = ram_data
			elseif bitx.band(address, 0x67) == 0x41 then
				next_char0_right = ram_data
			elseif bitx.band(address, 0x67) == 0x42 then
				next_range_h = bitx.band(ram_data, 0x3FF)
			elseif bitx.band(address, 0x67) == 0x43 then
				next_range_v = bitx.band(ram_data, 0x3FF)
			elseif bitx.band(address, 0x67) == 0x44 then
				next_cursor = bitx.band(ram_data, 0x3FF)
			elseif bitx.band(address, 0x67) == 0x45 then
				next_newline = bitx.band(ram_data, 0xFF)
			elseif bitx.band(address, 0x67) == 0x46 then
				next_color = bitx.band(ram_data, 0xFF)
			elseif bitx.band(address, 0x67) == 0x47 then
				next_scrollmask = bitx.band(ram_data, 0x1FFFFFFF)
		...
		return {
			...
			pixel_position   = bitx.bor(0x10000000, bitx.band(ram_data, 0xFFFF)),
			pixel_color      = bitx.bor(0x10100000, bitx.lshift(ram_addr, 4)),
			char_print       = bitx.bor(0x00000002, char_print),
			char_color       = bitx.bor(0x10000000, bitx.band(bitx.rshift(ram_data, 8), 0xFF)),
			char_char        = bitx.bor(0x10000000, bitx.band(ram_data, 0xFF)),
			char_config      = ram_addr,
			ram_data_prev    = inputs.ram_data,
			ram_addr_prev    = inputs.ram_addr,
		}
	end,
