...
return testbed.module(function(params)
	...
	return {
		...
		func = function(inputs)
			...
			local instr_4 = spaghetti.rshiftk(inputs.instr, 3)
			local rhs_ka_subtract = rhs_ka:bxor(0x3FFFFFFF):bxor(spaghetti.lshift(0x3FFFFFFF, instr_2:bor(instr_4):bor(0x10000):bsub(0xFFFE)))
		...
		fuzz_inputs = function(inputs)
			...
			if bitx.band(instr, 0x000F) == 0x000E then
		...
		fuzz_outputs = function(inputs)
			...
			local subtract_in = bitx.band(inputs.instr, 0x000A)
			...
			if params.for_mcore then
				...
				local check_27 = bitx.band(inputs.instr, 0x000F) == 0x000E
