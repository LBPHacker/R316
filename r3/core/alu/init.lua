...
return testbed.module(function(params)
	...
	return {
		...
		outputs = {
			...
			params.for_mcore and { name = "res_mulh", index = 7, keepalive = 0x10000000, payload = 0x0000FFFF } or nil,
		},
		func = function(inputs)
			...
			if params.for_mcore then
				...
				adder_pri, adder_sec = spaghetti.select(
					inputs.instr:bxor(0x0008):bxor(0x0004):bxor(0x0002):band(0x000F):zeroable(),
					inputs.pri, csmult_outputs.carries,
					inputs.sec, csmult_outputs.sums_high
				)
				adder_pri, adder_sec = spaghetti.select(
					inputs.instr:bxor(0x0008):band(0x000F):zeroable():bsub(1):zeroable(),
					adder_pri, csmult_outputs.carries,
					adder_sec, csmult_outputs.sums_high
				)
			end
		...
		fuzz_outputs = function(inputs)
			...
			local adder_sec = inputs.sec
			local res_mull, res_mulh
			local do_add = bitx.band(inputs.instr, 0x000C) == 0x0004
			if params.for_mcore then
				...
				if bitx.band(inputs.instr, 0x000F) == 0x000E or bitx.band(inputs.instr, 0x000E) == 0x0008 then
					adder_pri = carries
					adder_sec = sums_high
				end
			...
			return {
				res      = mux_outputs.muxed,
				res_high = mux_outputs.muxed_high,
				flags    = { value = flags_out, mask = do_add and 0xFFFFFFFF or 0xFFFFFFFC },
				res_mulh = (params.for_mcore and (bitx.band(inputs.instr, 0x0F) == 14 or
				                                  bitx.band(inputs.instr, 0x0E) == 8)) and res_mulh or false,
			}
		end,
	}
end)
