...
return testbed.module(function(params)
	return {
		...
		inputs = {
			...
			params.for_mcore and { name = "res_mull", index = 7, keepalive = 0x10000000, payload = 0x0000FFFF, initial = 0x10000000 } or nil,
		},
		outputs = {
		...
		fuzz_inputs = function()
			return {
				...
				res_mull  = params.for_mcore and bitx.bor(0x10000000, math.random(0x00000000, 0x0000FFFF)) or nil,
			}
		end,
		fuzz_outputs = function(inputs)
			local do_mul = bitx.band(inputs.instr, 0xE) == 14
			local keep_flags = bitx.band(inputs.instr, 0x8000) == 0 or (params.for_mcore and do_mul)
			...
			if params.for_mcore and do_mul then
