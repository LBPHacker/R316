	...
	return {
		...
		func = function(inputs)
			...
			if not params.for_mcore then
				...
				keep_old = keep_old:bor(flags_not_mul:band(flags_not_mullh):bxor(1))
			end
			local state, curr_instr, curr_imm, pc, flags, ram_addr, ram_data, wreg_addr = spaghetti.select(
				keep_old:band(1):zeroable(),
		...
		fuzz_outputs = function(inputs)
			local wait = bitx.band(inputs.io_state, 1) ~= 0
			local keep_old = wait
			if not params.for_mcore then
				keep_old = keep_old or bitx.band(inputs.instr, 0x0F) == 14 or
